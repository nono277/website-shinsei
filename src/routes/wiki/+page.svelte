<script lang="ts">
	...
	type Section = 'lore' | 'classes' | 'rangs' | 'factions' | 'guildes' | 'donjons' | 'pvp';
	...
	const sections: { id: Section; label: string; icon: string; desc: string }[] = [
		...
		{ id:'classes',  label:'Classes',  icon:'⚔️',  desc:'5 voies de combat'    },
	...
	const rankShowcase = [
		{ label:'D',  icon:rankD,  color:'#6b7280', name:'Dormant'    },
		{ label:'C',  icon:rankC,  color:'#22c55e', name:'Éveillé'    },
		{ label:'B',  icon:rankB,  color:'#3b82f6', name:'Briseur'    },
		{ label:'A',  icon:rankA,  color:'#a855f7', name:'Fléau / Transcendant' },
		{ label:'S',  icon:rankS,  color:'#f59e0b', name:'Souverain'  },
		{ label:'SS', icon:rankSS, color:'#ef4444', name:'Abyssal SS' },
	];
	...
	const pvpModes = [
		{ title:'Zones PvP Libres',       icon:'🗡️', color:'#ef4444',
		  desc:"Toutes les zones Rang B et supérieur sont des zones PvP ouvertes. Mourir entraîne une perte de 10% de l'XP et la possibilité pour le vainqueur de looter votre équipement le moins précieux." },
		{ title:'Arènes Classées',        icon:'🏟️', color:'#7c3aed',
		  desc:"Arènes 1v1, 3v3 et 5v5 disponibles dans chaque zone sécurisée. Classement saisonnier mis à jour en temps réel. Le top 10 reçoit des récompenses cosmétiques exclusives et un titre animé." },
		{ title:'Guerres de Factions',    icon:'⚔️', color:'#f59e0b',
		  desc:"Deux fois par semaine, L'Ordre, Les Fracturés et Les Nomades s'affrontent pour le contrôle de territoires stratégiques. 50 joueurs max par faction, victoire par points de capture." },
		{ title:'Jugement des Corrompus', icon:'👁️', color:'#06b6d4',
		  desc:"Un Éveillé qui tue 5 civils en 24h est déclaré Corrompu. Prime mondiale sur sa tête, accès aux zones sécurisées bloqué 48h, son pseudo s'affiche en rouge sur la carte mondiale." },
	];
</script>
	...
	" class="lg:flex lg:flex-col">
		...
		<div style="padding:1rem 1.25rem;border-top:1px solid #1e1530;">
			<p style="font-family:'Share Tech Mono',monospace;font-size:0.6rem;color:#1f2937;line-height:1.5;">
				SHINSEI 新世 · Wiki v1.0<br/>Dernière màj : Mai 2026
			</p>
		</div>
	</aside>
			...
			<div transition:fade={{ duration:160 }}>
				...
				{:else if activeSection === 'guildes'}
					...
					<div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;margin-bottom:2rem;">
						{#each [
							{ label:'Rejoindre une guilde', req:'Rang Briseur C', color:'#3b82f6' },
							{ label:'Créer une guilde',     req:'Rang Fléau B + 5 000 pièces', color:'#f59e0b' },
						] as item}
					...
					<div style="margin-bottom:2rem;padding:1.25rem;background:#0d0d15;border:1px solid #1e1530;border-radius:0.75rem;">
						<h3 style="font-family:'Rajdhani',sans-serif;font-weight:900;font-size:1.1rem;color:#06b6d4;margin-bottom:0.75rem;">TERRITOIRE — CHUNKS DISPONIBLES</h3>
						...
						<p style="font-size:0.78rem;color:#374151;line-height:1.5;">Dans le territoire d'une guilde : les membres construisent librement, les ennemis ne peuvent pas casser les blocs, un coffre partagé est accessible aux membres.</p>
					</div>

					<!-- Guerre de guildes -->
					<div style="margin-bottom:2rem;padding:1.25rem;background:linear-gradient(135deg,#0d0d15,#ef444408);border:1px solid #ef444425;border-radius:0.75rem;">
						<h3 style="font-family:'Rajdhani',sans-serif;font-weight:900;font-size:1.1rem;color:#ef4444;margin-bottom:0.75rem;">GUERRE DE GUILDES</h3>
						<p style="font-size:0.85rem;color:#94a3b8;line-height:1.65;margin-bottom:0.75rem;">
							Disponible au rang <strong style="color:white;">Transcendant A</strong>. Le fondateur ou un officier peut déclarer la guerre à une guilde d'une faction ennemie.
						</p>
						<div style="display:flex;flex-wrap:wrap;gap:0.4rem;">
							{#each ['Membres attaquables partout (zone orange)', 'Durée max : 7 jours', 'Fin : capitulation ou expiration', 'Récompense : points de faction'] as point}
								<span style="font-size:0.72rem;color:#ef4444;background:#ef444415;padding:0.2rem 0.6rem;border-radius:0.25rem;border:1px solid #ef444430;">{point}</span>
							{/each}
						</div>
					</div>
				...
				{:else if activeSection === 'pvp'}
					<p style="font-size:0.9rem;color:#94a3b8;line-height:1.7;margin-bottom:2rem;">
						Le PvP dans SHINSEI est une composante centrale du gameplay. Il est autorisé dans toutes les zones non-sécurisées et obligatoire dans les zones de guerre. Le rang PvP est séparé du rang de progression.
					</p>
					...
					">
						...
						<div>
							...
							<p style="font-size:0.8rem;color:#64748b;line-height:1.55;">Le PvP est désactivé dans toutes les zones Rang D et dans le hub de départ. Les nouvelles recrues sont protégées pendant leurs 24 premières heures de jeu.</p>
						</div>
					</div>
				{/if}
